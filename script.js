/* =========================================================
   ...
   MAIN SCRIPT - FULL & FIXED VERSION
   ========================================================= */
...
let toastTimer;
const WHATSAPP_NUMBER = "918009174690"; // Updated WhatsApp Number
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    console.log("✅ Ubaid Medical Store V2 JS Fully Ready");
});
...
function initSearch() {
    ...
    document.querySelectorAll(".header-search button, .mobile-search-box button, .search-main-btn")
        .forEach(button => {
            button.addEventListener("click", event => {
                event.preventDefault();
                const parent = button.closest(".header-search, .mobile-search-box, .search-box");
...
function initCart() {
    document.querySelectorAll(".add-cart-btn").forEach(button => {
        button.addEventListener("click", () => {
            ...
            // Fixed Price extraction logic
            const priceElement = card.querySelector(".product-price strong, .price, .product-price");
...
function updateCartTotal() {
    ...
    if (summary) summary.textContent = `${itemCount} item${itemCount !== 1 ? "s" : ""}`;
}
...
function closeCart() {
    ...
    if (overlay && !document.querySelector(".nav.open")) overlay.classList.remove("active");
    document.body.classList.remove("no-scroll");
}
...
function initButtons() {
    // Both themeToggle ID and attribute triggers support
